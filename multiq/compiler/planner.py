...
class Planner:
    ...
    def set_input_circuits(self, input_circuits: list[str], optimization_level: int = 3) -> None:
        ...
        for i in input_circuits:
            ...
            circuit = transpile(circuit, basis_gates=['cz', 'rz', 'ry', 'rx'], optimization_level=optimization_level)
            dag = circuit_to_dag(circuit)
            self.circuit_files.append(i)
            self.input_circuits.append(circuit)
            self.input_dags.append(dag)
            self.input_layers.append(self._split_DAG_into_execution_layers(dag, self.config.layer_split_window))
    ...
    def _split_DAG_into_execution_layers(self, dag: DAGCircuit, window_size) -> list[Any]:
        ...
        while len(dag_layers) > 0:
            window_slice = self._fetch_window_from_zero(dag_layers, window_size)
    ...
    def _largest_entanglement_op(self, execution_layers: list[list]) -> int:
        ...
        for layer in execution_layers:
            n_entanglement = len([i for i in layer if isinstance(i, DAGOpNode) and i.num_qubits >= 2])
    ...
    def set_best_architectures(self) -> List[List[Any]]:
        ...
        for circuit, dag, circuit_file in zip(self.input_circuits, self.input_dags, self.circuit_files):
            ...
            minimun_storage_width = max(minimun_entanglement_width, (ceil(circuit.num_qubits / storage_rows)-1) * storage_atom_spacing)
            ...
            out = self._generate_arch_json(
                ...
                entanglement_cols=int((selected_storage_width+entanglement_pair_spacing) // (entanglement_atom_spacing+entanglement_pair_spacing)),
                storage_rows=storage_rows,
                storage_cols=int(selected_storage_width // storage_atom_spacing)
            )
